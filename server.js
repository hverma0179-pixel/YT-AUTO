...
async function prepareVideoAssets(job) {
  ...
  await runTool("yt-dlp", ["-f", "bv*+ba/b", "--merge-output-format", "mp4", "-o", rawPath, job.videoUrl]);
...
async function requireTool(name) {
  try {
    await runTool(name, ["--version"], { maxOutput: 2000 });
  } catch (error) {
...
async function logToolStatus() {
  for (const name of ["ffmpeg", "yt-dlp"]) {
    try {
      const output = await runTool(name, ["--version"], { maxOutput: 300 });
