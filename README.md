...
- Downloads a Shorts-friendly source, cuts a short segment first, then renders only that clip with `yt-dlp` and `ffmpeg`.
...
2. Cuts a short test clip first.
3. Renders only that clip to vertical Shorts format.
4. Uploads the final short with YouTube resumable upload chunks.
...
The default ffmpeg render target is 720x1280 with `libx264`, `ultrafast`, `crf 30`, AAC audio at 128k, and `+faststart`.
